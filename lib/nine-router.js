...
export class NineRouterImageProvider {
  constructor(options = {}) {
    this.name = "9router";
    this.token = options.token || process.env.NINEROUTER_API_KEY || process.env.NINEROUTER_KEY || "";
    this.model = options.model ?? process.env.NINEROUTER_IMAGE_MODEL ?? "";
    this.apiBase = String(options.apiBase || process.env.NINEROUTER_URL || DEFAULT_API_BASE).replace(/\/+$/, "");
  ...
  get endpoint() {
    const versionedBase = /\/v1$/i.test(this.apiBase) ? this.apiBase : `${this.apiBase}/v1`;
    return `${versionedBase}/images/generations?response_format=binary`;
  }
